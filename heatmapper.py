...
class traj_analysis:
    ...
    def cont_per_frame(self,frame_index,cont_dist,carbon,csv_filename): # <--- The actual function which is executed on each CPU
    ...
    def cont_pro(self,cont_dist=3.3,carbon=True,start=0,stop=-1,skip=1):
        ...
        for segid in self.analyte_segids: # <--- Iterate through chains
            ...
            csv_filename = 'heat_contacts_'+str(self.probe_sel).replace(' ','_')+'_to_'+str(self.analyte_sel).replace(' ','_')+'_segid_'+str(segid)
            df = pd.DataFrame({'resids':self.analyte_resids})
            df.to_csv(csv_filename,index=False)
            run_per_frame = partial(self.cont_per_frame, cont_dist=cont_dist,carbon=carbon,csv_filename=csv_filename) # <--- Set the per frame function
            ...
            print('Running analysis - Start time: ',str(datetime.now().strftime("%H:%M:%S")))
            ...
            print('Analysis done - Stop time: ',str(datetime.now().strftime("%H:%M:%S")))
    ...
    def gen_map(self,df_data,res_op=1,res_rep='spacefill',res_rad=0,BB_only=True):
        ...
        act = self.mda_universe.trajectory[-1].select_atoms(self.analyte_sel)
